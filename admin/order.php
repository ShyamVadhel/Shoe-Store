				...
				<div style='width:975px;' class="alert alert-info">
					  <table class="table table-hover">
						<thead>
						  ...
						  <tbody>
							<?php
							$Q1 = $con->query("SELECT * FROM transaction WHERE order_stat = 'Confirmed'");
							while($r1 = $Q1->fetch_array()){

							$tid = $r1['transaction_id'];

							$Q2 = $con->query("SELECT * FROM transaction_detail LEFT JOIN product ON product.product_id = transaction_detail.product_id WHERE transaction_detail.transaction_id = '$tid' ");
							$r2 = $Q2->fetch_array();

							$pid = $r2['prev(array)product_id'];
							$o_qty = $r2['order_qty'];
							...
							$Q3 = $con->query("SELECT sum(amount) FROM transaction WHERE order_stat = 'Confirmed'");

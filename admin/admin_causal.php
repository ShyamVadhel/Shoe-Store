...
<body>
	...
	<br>
				...
				<div class="modal-body">
					...
					<center>
						<table>
							...
							<tr>
								<td><input type="hidden" name="category" value="causal"></td>
							</tr>
						</table>
					</center>
				</div>
		...
		<?php
			if (isset($_POST['add']))
				...
				$q1 = $con->query("INSERT INTO product ( product_id,product_name, product_price, product_size, product_image, brand, category)
				VALUES ('$product_code','$product_name','$product_price','$product_size','$name', '$brand', '$category')");

				$q2 = $con->query("INSERT INTO stock ( product_id, qty) VALUES ('$product_code','$qty')");

				header ("location:admin_causal.php");
			}}
		}
	...
	<div id="rightcontent" style="position:absolute; top:10%;">
			<div class="alert alert-info"><center><h2>Causals</h2></center></div>
			<br />
			...
			<table class="table table-hover" style="background-color:;">
				...
				<?php
					...
					$query = $con->query("SELECT * FROM `product` WHERE category='causal' ORDER BY product_id DESC") or die(mysqli_error());
				...
				<tr class="del<?php echo $id?>">
					...
					<?php
					$query1 = $con->query("SELECT * FROM `stock` WHERE product_id='$id'") or die(mysqli_error());
...
<?php
 ...
 $row = $result->fetch_array();
  ...
  $que = $con->query("UPDATE `stock` SET `qty` = '$total' WHERE `product_id`='$pid'") or die(mysql_error());
  echo "<script>window.location = 'admin_casual.php'</script>";
  //header("Location:admin_casual.php");
 }
 ...
 $result = $con->query("SELECT * FROM `stock` WHERE product_id='$pid'") or die(mysqli_error());
 $row = $result->fetch_array();
  ...
  $que = $con->query("UPDATE `stock` SET `qty` = '$total' WHERE `product_id`='$pid'") or die(mysqli_error());

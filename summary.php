		...
		<label>Shoe Store</label>
			...
			<?php
				$id = (int) $_SESSION['id'];
					...
					$query = $con->query ("SELECT * FROM customer WHERE customerid = '$id' ");
...
<br>
	...
	<div id="profile" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="width:700px;">
				<div class="modal-header">
					...
					<h3 id="myMo/dalLabel">My Account</h3>
				</div>
...
<div id="container">
	...
	<label style="font-size:25px;">Summary of Order/s</label>
		...
		<?php
		$t_id = $_GET['tid'];
		$query = $con->query("SELECT * FROM transaction WHERE transaction_id = '$t_id'");
		$fetch = $query->fetch_array();

		$amnt = $fetch['amount'];
		$t_id = $fetch['transaction_id'];

		$query2 = $con->query("SELECT * FROM transaction_detail LEFT JOIN product ON product.product_id = transaction_detail.product_id WHERE transaction_detail.transaction_id = '$t_id'");
	...
	<a href="confirm.php"><button type="button" class="close" aria-hidden="true">Confirm</button></a>
	<!-- </form>
